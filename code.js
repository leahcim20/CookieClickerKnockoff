...
function SetDataDefault() {
    gameinfo = {cookies: 0, taxes: 1.15, clickpower: 1, cps : 0}
    upgrades = [
    {bp: 15,  value: 0.1},
    {bp: 100,  value: 1},
    {bp: 1100,  value: 8},
...
for (let i = 0; i < upgrades.length; i++) {
    ...
    btn.id = upgrades[i].name.toLowerCase().replace(/\s+/g, "") + "sell"
    btncontainer.appendChild(btn)
    upgrades[i].sell = document.getElementById(btn.id)
    upgrades[i].sell.addEventListener("click", () => Sell(i))
}
...
function Update() {
    for (let i = 0; i < upgrades.length; i++) {
        ...
        upgrades[i].sell.innerText = "Sell"
    }
    text.innerText = formatNumber(gameinfo.cookies)
...
function Sell(i) {
    if (upgrades[i].amount > 0) {
        gameinfo.cookies += Math.round(upgrades[i].price / gameinfo.taxes)
        upgrades[i].amount -= 1
        upgrades[i].price = Math.round(upgrades[i].price / gameinfo.taxes)
        gameinfo.cps -= upgrades[i].value
        Update()
    }
}
...
function CookieClicked() {
    gameinfo.cookies += gameinfo.clickpowe
    Update()
}
...
document.getElementById("fix").addEventListener("click", () => {
    gameinfo.cookies = Math.round(gameinfo.cookies)
    Update()
})